...
def validate_transferts_marchandise(doc, method):
	"""Validation principale lors de la sauvegarde du document."""
	_validate_delivery_notes(doc)

def on_submit_transferts_marchandise(doc, method):
	"""Actions à exécuter lorsque le document est soumis."""
	_update_preparation_status(doc)

def on_cancel_transferts_marchandise(doc, method):
	"""Réinitialise les bons de livraison comme non préparés et retourne les articles au dépôt d'origine."""
	_reset_preparation_status(doc)
	_return_items_to_source(doc)

def _validate_delivery_notes(doc):
...
def _update_preparation_status(doc):
...
def _reset_preparation_status(doc):
...
def _return_items_to_source(doc):
