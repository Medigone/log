...
def livraison_query_conditions(user=None):
	...
	roles = _roles(user)
	if roles & {"System Manager", "Responsable", "Planificateur"}:
		return None
	...
	return (
		...
		"AND `tabLivraison`.`etat_planification` IN ('Publiée', 'En cours', 'Terminée')"
	)
...
def livraison_has_permission(doc, user=None, permission_type=None):
	...
	return bool(
		...
		and doc.etat_planification in {"Publiée", "En cours", "Terminée"}
	)
...
def paiement_query_conditions(user=None):
	user = user or frappe.session.user
	roles = _roles(user)
	if roles & {"System Manager", "Responsable"}:
		return None
	if "Livreur" in roles:
