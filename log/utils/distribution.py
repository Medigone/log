	...
	with db_txn():
		...
		if mode == "manuel" and not manual_assignments:
			...
			for commune_id, bons_liste in par_commune.items():
				nb_colis_total = sum(bon.total_qty or 0 for bon in bons_liste)
		...
		if mode == "manuel" and manual_assignments and isinstance(manual_assignments, dict):
			...
			for commune, livreur_id in manual_assignments.items():
				...
				repartition[livreur_id]["total_colis"] += sum(bon.total_qty or 0 for bon in bons_commune)
		else:
					...
					if livreurs_eligibles:
						...
						if livreur_optimal:
							...
							if reste == 0:
								...
								bons_restants = sorted(bons_liste, key=lambda x: x.total_qty or 0, reverse=True)
								...
								for livreur_id_split, nb_colis_attribue in attribution_split:
									...
									for bon in bons_restants:
										if colis_attribues + (bon.total_qty or 0) <= nb_colis_attribue:
											rep["bons_de_livraison"].append(bon)
											colis_attribues += bon.total_qty or 0
...
def diagnostiquer_distribution_bons(date_livraison):
	...
	for bon in bons_eligibles:
		par_commune.setdefault(bon.commune, []).append(bon)
		total_colis += bon.total_qty or 0
	...
	return {
		...
		"detail_par_commune": {
			commune: {
				"nb_bons": len(bons),
				"nb_colis": sum(b.total_qty or 0 for b in bons),
...
def valider_distribution_v2(date_livraison):
	...
	try:
		livraisons = frappe.get_all("Livraison",
			...
			fields=["name", "livreur", "nombre_bons_de_livraison", "status"]
		)
		
		resultats_validation = []
		
		for livraison in livraisons:
			# Compter les bons de livraison réels
			colis_reels = frappe.db.count("Livraison Bon de Livraison", {"parent": livraison.name})
			...
			resultats_validation.append({
				...
				"nb_bls": livraison.nombre_bons_de_livraison or 0,
				"nb_reels": colis_reels,
				"nb_bons": bons_count,
				"coherent": (livraison.nombre_bons_de_livraison or 0) == colis_reels
			})
		...
		for livreur_sous_utilise, info_sous_utilise in livreurs_sous_utilises:
			...
			for commune in repartition_equilibree[livreur_surcharge]["communes"]:
				...
				if livreur_sous_utilise in livreurs_couvrant_commune(commune):
					...
					colis_commune = sum(
						bon.get("total_qty", 0) if isinstance(bon, dict) else (bon.total_qty or 0)
