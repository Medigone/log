...
import { LogOut, User, Truck, Home, RefreshCw, Package, ClipboardList } from "lucide-react";
...
function AppContent() {
  ...
  useEffect(() => {
    ...
    const blParam = urlParams.get("bl") || urlParams.get("colis");
    const publicParam = urlParams.get("public");

    if (blParam && (publicParam === "1" || !currentUser)) {
  ...
  if (isPublicAccess) {
    ...
    const blParam = urlParams.get("bl") || urlParams.get("colis");
    if (blParam) {
      return <ColisPublicView colisId={blParam} />;
    }
  }
        ...
        <div className="flex-1 md:ml-52">
          <div className="max-w-6xl mx-auto px-4 pt-16 md:pt-4 pb-6">
            <Routes>
              ...
              <Route path="/bl/:id" element={<ColisDetailsRoute />} />
              {/* Fallback route */}
              <Route path="*" element={<HomePage />} />
            </Routes>
          </div>
...
function LivraisonDetailsRoute() {
  ...
  return (
    <LivraisonDetails
      ...
      onColisSelect={(blId) => navigate(`/bl/${blId}?from=livraison&livraisonId=${id}`)}
    />
  );
}
...
function ColisDetailsRoute() {
  ...
  const { data: colisData, isLoading } = useFrappeGetDoc("Delivery Note", id || undefined);
