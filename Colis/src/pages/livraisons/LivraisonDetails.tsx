...
const LivraisonDetails = ({ livraisonId, onBack, onColisSelect }: LivraisonDetailsProps) => {
  ...
  const { data: blData, mutate: mutateBl } = useFrappeGetDocList<any>('Delivery Note', {
    fields: [
      'name',
      'custom_statut',
      'customer',
    ...
    filters: livraisonBonsLivraison.length > 0 ? [['name', 'in', livraisonBonsLivraison.map(bon => bon.bon_de_livraison)]] : [],
    limit: 1000
  });
  ...
  const hasLegacyNouveau = (blData || []).some(
    (bl) => (bl.custom_statut || bl.status || "Nouveau") === "Nouveau"
  );

  // Calcul des statistiques
  const stats = React.useMemo(() => {
    const colisToUse = (blData || []).map((bl) => ({
      ...bl,
      status: bl.custom_statut || bl.status,
      client: bl.customer_name || bl.customer,
      bl: bl.name,
      total_art: bl.total_qty,
    }));
          ...
          <div className="hidden md:flex px-4 py-3 items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              >
                ...
                <span className="text-sm font-medium">Préparer les bons</span>
              </button>
              )}
          ...
          {livraisonBonsLivraison.length > 0 ? (
            <div className="space-y-4 p-4">
              {livraisonBonsLivraison.map((bonLivraison) => {
                  const colisDuBon = (blData || [])
                    .filter((bl) => bl.name === bonLivraison.bon_de_livraison)
                    .map((bl) => ({
                      ...bl,
                      status: bl.custom_statut || bl.status,
                      client: bl.customer_name || bl.customer,
                  ...
                  >
                        ...
                        <div className="mb-3 flex items-center justify-between">
                          ...
                          <div className="flex items-center gap-2">
                            <span className="text-xs text-muted-foreground">
                              {colisDuBon.length} bon{colisDuBon.length > 1 ? "s" : ""}
                            </span>
                            {isExpanded ? (
                    ...
                    {isExpanded && colisDuBon.length > 0 ? (
                      ...
                      <div className="p-6 text-center animate-in fade-in duration-300">
                        ...
                        <span className="text-sm text-muted-foreground">Aucun détail pour ce bon de livraison</span>
                      </div>
                    ) : null}
                  </div>
                );
