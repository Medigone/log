...
const ColisDetails = ({ colisId, livraisonId, onBackToLivraison }: ColisDetailsProps) => {
  ...
  const { data, mutate, error, isLoading } = useFrappeGetDoc<any>(
    "Delivery Note",
    colisId
  );
  ...
  useEffect(() => {
    if (data) {
      const mapped: ColisData = {
        ...data,
        id: data.name,
        name: data.name,
        status: data.custom_statut || data.status,
        client: data.customer_name || data.customer,
        bl: data.name,
        photo_livraison: data.custom_photo_livraison,
        commentaire_livreur: data.custom_commentaire_livreur,
      ...
      setLocalColisData(mapped);
      setEditingArticle(null);
      setTempQuantities({});
      setCapturedPhoto(mapped.photo_livraison || null);
      setCommentText(mapped.commentaire_livreur || "");
    }
  ...
  const saveToBackend = async (updatedData: Partial<ColisData>) => {
    ...
    try {
      ...
      if (updatedData.status) payload.custom_statut = updatedData.status;
      if (updatedData.commentaire_livreur !== undefined) {
        payload.custom_commentaire_livreur = updatedData.commentaire_livreur;
      }
      await updateColis("Delivery Note", colisId, payload);
      await mutate();
    } catch (e) {
      console.error(e);
  ...
  const uploadPhoto = async (
    ...
    const r = await fetch(
      "/api/method/log.delivery_note_ops.upload_photo_livraison",
      {
        method: "POST",
        headers: {
  ...
  const deletePhoto = async () => {
    ...
    try {
      const csrfToken = (window as any).csrf_token;
      const r = await fetch(
        "/api/method/log.delivery_note_ops.delete_photo_livraison",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
