...
import { plural } from "./DeliveriesKpis";
...
/**
 * Stops of the expanded route, rendered INSIDE the routes table (under its row),
 * replacing the separate "Arrêts · TR-…" card at the bottom of the page:
 * the link between the clicked row and that card was invisible without scrolling.
 */
export function StopsSubTable({
  ...
  /** Push the failed BL back to planning. */
  ...
  return (
    <div className="flex flex-col gap-2 bg-muted/20 px-3 pb-3.5 pt-2.5 pl-[34px]">
        ...
        <div className="grid grid-cols-[44px_minmax(0,1.3fr)_minmax(0,1fr)_118px_96px_76px] border-b bg-muted/40 px-2.5">
          {["N°", "Client · BL", "Lieu", "Statut", "Heure", "Carte"].map((header, index) => (
            <div
              key={header}
              className={cn(
                "t-micro py-1.5 text-muted-foreground",
                index === 5 && "text-right",
              )}
            >
              {header}
          ...
          return (
            ...
            >
              ...
              <div className="min-w-0 py-2 pr-2">
                ...
                <p className="truncate font-mono text-[11px] text-muted-foreground">{stop.deliveryNote}</p>
              </div>
              ...
              <div className="whitespace-nowrap py-2 font-mono text-[11.5px] text-muted-foreground">
                {stop.completedAt ? String(stop.completedAt).slice(11, 16) : "—"}
              </div>
      ...
      {failed.length > 0 && (
        <div className="flex items-center gap-2 rounded-lg border border-red-200 bg-red-50/40 px-2.5 py-2">
          <span className="size-1.5 shrink-0 rounded-full bg-red-600" />
          <span className="min-w-0 text-xs text-red-900">
