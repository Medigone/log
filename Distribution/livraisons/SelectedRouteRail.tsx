...
import type { RouteEvent } from "./routeProgressColumns";
...
/**
 * Right rail. When nothing is selected it collapses to the hint only —
 * no primary button without a target (that was a defect in the first mock pass).
 */
export function SelectedRouteRail({
  ...
  /** 3 most recent field events. Not in the API today — see README. */
  events?: RouteEvent[];
  /** Estimated end of tour. Hide the line when unknown rather than guessing. */
  eta?: string;
  ...
  return (
    <Card>
      <CardHeader className="flex-row items-center justify-between gap-2">
        <CardTitle className="font-mono text-sm">{route.name}</CardTitle>
      ...
      <CardContent className="flex flex-col gap-3">
        <div>
          ...
          <p className="text-[11.5px] text-muted-foreground/70">
            {route.vehicleLabel || "Véhicule non affecté"} · départ {(route.plannedStart ?? "").slice(11, 16) || "—"}
          </p>
        </div>

        <div className="flex flex-col gap-1.5">
          <div className="flex items-baseline gap-1.5">
            <span className="whitespace-nowrap text-[22px] font-semibold leading-none tracking-tight tabular-nums">
            ...
            {events.slice(0, 3).map((event, index) => (
              <div key={`${event.label}-${index}`} className="flex items-start gap-2">
                ...
                <div className="min-w-0">
                  ...
                  <p className="font-mono text-[10.5px] text-muted-foreground">{event.age}</p>
                </div>
              </div>
            ))}
          </div>
