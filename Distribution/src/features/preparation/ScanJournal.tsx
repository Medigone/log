...
import type { ScanLogEntry } from "@/features/preparation/pickScan";
  ...
  return (
    <section className="overflow-hidden rounded-xl border bg-card" aria-label="Journal des scans">
      <div className="flex items-center gap-2 border-b px-3 py-2.5">
        ...
        <span className="num rounded bg-muted px-1.5 py-0.5 text-[11px] text-muted-foreground">{log.length}</span>
      ...
      {log.map((entry) => (
        ...
        >
          <div className="num py-2 text-[11.5px] text-muted-foreground">{entry.time}</div>
          <div className={cn("min-w-0 truncate py-2 pr-2 text-[12.5px]", TONE_TEXT[entry.tone])}>{entry.label}</div>
          <div className="num truncate py-2 text-[11.5px] text-muted-foreground">{entry.code}</div>
          <div className="num py-2 text-[11.5px] text-muted-foreground">
