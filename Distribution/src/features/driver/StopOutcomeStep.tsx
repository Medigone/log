...
/**
 * Étape 1 — un tap suffit : la sélection avance l’étape.
 */
export function StopOutcomeStep({
  ...
  const options: Array<{
  ...
  }> = [
    {
      ...
      hint: `${stop.totalQuantity} articles · ${formatDriverMoney(stopExpectedAmount(stop), false)}`,
  ...
  return (
    <div className="flex flex-col gap-3.5">
      <div>
        ...
        <p className="mt-0.5 text-sm text-muted-foreground">Un tap suffit — l’étape suivante s’ouvre automatiquement.</p>
      </div>
        ...
        {options.map((option) => (
          <button
            ...
            className={cn(
              ...
              option.primary
                ? "h-[5.25rem] bg-emerald-600 text-white hover:bg-emerald-700"
                : "h-[4.5rem] border-[1.5px] border-border bg-background hover:border-brand-300 hover:bg-brand-50/40",
            )}
          >
            <span
              className={cn(
      ...
      <div className="flex items-center gap-2.5 rounded-lg border bg-background px-3.5 py-2.5">
        <span
          className={cn(
            "size-2 rounded-full",
            gpsAccuracy != null ? "animate-pulse bg-emerald-500" : locating ? "animate-pulse bg-amber-500" : "bg-amber-500",
          )}
        />
