...
/**
 * Remplace l’ancienne étape « Récapitulatif » : rendu en bas de l’étape Encaissement.
 * Photo et signature sont facultatives : elles n’apparaissent que si collectées
 * et ne déclenchent jamais « Éléments manquants ».
 */
  ...
  const items: CollectedItem[] = [
    {
      ...
      detail: hasGps && gpsAccuracy != null ? `${Math.round(gpsAccuracy)} m` : hasGps ? "Acquise" : "Manquante",
    },
  ];
  if (hasPhoto) {
    items.push({ label: "Photo de livraison", done: true, onReview: onReviewPhoto });
  }
  if (hasSignature) {
    items.push({ label: "Signature", done: true, onReview: onReviewSignature });
  }
  const allDone = hasGps;
      ...
      <ul>
        {items.map((item) => (
          <li key={item.label} className="flex items-center gap-2.5 border-b px-4 py-2.5 last:border-b-0">
            <span
              className={cn(
                "flex size-[1.375rem] items-center justify-center rounded-full",
