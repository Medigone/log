  ...
  return (
    <div className="flex flex-col gap-2 border-b bg-muted/40 px-3 py-2.5">
      <div className="flex items-center gap-2">
        <span className="text-[10.5px] tracking-wider text-muted-foreground uppercase">À planifier</span>
...
export function useBacklogGroups(items: KanbanBLItem[], group: BacklogGrouping): BacklogGroup[] {
  return useMemo(() => {
    ...
    const keyOf = (item: KanbanBLItem) =>
      group === "date" ? item.assignment.requestedDate || "Sans date" : item.assignment.wilaya || "Sans wilaya";
    const order: string[] = [];
