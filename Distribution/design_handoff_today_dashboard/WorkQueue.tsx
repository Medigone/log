...
export interface QueueItem {
  id: string;
  ref: string;
  /** Référence raccourcie affichée sur les cartes Kanban */
  short: string;
  client: string;
  city?: string;
  date?: string;
...
export function buildQueueItems(data: ActivityDashboardData | undefined, today: string): QueueItem[] {
  ...
  for (const pick of data?.preparation?.pickLists ?? []) {
    items.push({
      id: pick.name,
      ref: pick.name,
      short: pick.name.replace(/^CMD-\d{4}-/, "CMD "),
  ...
  for (const note of data?.dispatch?.notes ?? []) {
    ...
    items.push({
      ...
      short: note.deliveryNote.replace(/^MAT-DN-\d{4}-/, "BL "),
      client: note.customerName,
      city: note.customerCity,
  ...
  for (const route of data?.fulfillment?.toLoadRoutes ?? []) {
    items.push({
      ...
      short: route.vehicleLabel || route.name,
...
export function WorkQueue({
  ...
  const [selection, setSelection] = useState<Record<string, true>>(
    () => Object.fromEntries((presetSelection ?? []).map((id) => [id, true as const])),
  );
  ...
  return (
    <Card className="overflow-hidden">
      <CardHeader className="flex-row items-center gap-2.5 border-b border-hairline">
        ...
        <Badge variant="secondary">{all.length} bons</Badge>
      ...
      <div className="flex flex-wrap items-center gap-2 border-b border-hairline px-3 py-2.5">
        ...
        <button type="button" className={chip(readyOnly)} onClick={() => setReadyOnly((v) => !v)}>
          <span className="size-1.5 rounded-full bg-warning" /> Préparé
        </button>
        <div className="flex-1" />
      ...
      {selectedIds.length > 0 && (
        <div className="flex flex-wrap items-center gap-2.5 bg-foreground px-3 py-2 text-background">
          ...
          <div className="flex-1" />
          <Button size="sm" variant="secondary" onClick={() => onCreateRoute?.(selectedIds)}>
            Créer une tournée
          </Button>
          <Button size="sm" variant="ghost" className="text-background hover:text-background">
      ...
      {rows.length === 0 ? (
                  ...
                  {cards.map((card) => (
                    ...
                    >
                      <div className="flex items-center gap-1.5">
                        <span className="num truncate text-[11.5px] font-medium">{card.short}</span>
                        <div className="flex-1" />
                        <span
                          className={cn(
                      ...
                      {/* whitespace-nowrap obligatoire : sinon la charge se casse lettre par lettre */}
        ...
        <div>
          <div className="grid grid-cols-[34px_minmax(0,1.25fr)_minmax(0,1fr)_92px_108px_88px_34px] border-b border-hairline bg-muted/40 px-3 num text-[10px] uppercase tracking-[0.06em] text-muted-foreground">
            ...
            <span className="py-1.5 text-right">Lignes / kg</span>
            <span className="py-1.5" />
          </div>
          {rows.map((row) => (
            <div
              ...
              className={cn(
                "grid cursor-pointer grid-cols-[34px_minmax(0,1.25fr)_minmax(0,1fr)_92px_108px_88px_34px] items-center border-b border-hairline px-3 hover:bg-muted/40",
