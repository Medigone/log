...
import type { PickLine, ScanTone } from "./usePickScan";
...
/**
 * Desktop scan console — the primary element of the pick step.
 * The camera stays mobile-only (BarcodeScannerDialog via PickFloorView): here the
 * handheld scanner types into this input, which keeps focus between reads.
 *
 * ⚠ Layout: the two panels use `repeat(auto-fit, minmax(272px, 1fr))` so they stack
 * below ~560px. A previous version used a fixed 300px secondary track, which starved
 * the primary column down to 237px and clipped both the placeholder and the hint.
 */
export function ScanConsole({
  ...
  /** Dev/demo affordance only — drop in production. */
  testCodes?: string[];
}) {
  ...
  return (
    <section
      className={cn(
        ...
        feedback.tone === "error" && "border-red-200",
      )}
    ...
    >
      <div className="grid" style={{ gridTemplateColumns: "repeat(auto-fit, minmax(272px, 1fr))" }}>
        <div className="flex min-w-0 flex-col gap-2.5 border-r p-3.5">
          <div className="flex min-w-0 items-center gap-2">
            ...
            <span className="whitespace-nowrap rounded bg-muted px-1.5 py-0.5 font-mono text-[10.5px] text-muted-foreground">
              +{step} / scan
            </span>
          </div>
          ...
          <Input
            ...
            className={cn(
              ...
              feedback.tone === "error" && "border-red-200 bg-red-50/40",
            )}
          />
          ...
          {testCodes && testCodes.length > 0 && (
            <div className="flex flex-wrap items-center gap-1.5">
              <span className="whitespace-nowrap text-[11.5px] text-muted-foreground">Test :</span>
              {testCodes.map((code) => (
                <button
                  key={code}
                  type="button"
                  onClick={() => onScan(code)}
                  className="h-6 whitespace-nowrap rounded-md border px-2 font-mono text-[11px] text-muted-foreground hover:bg-muted"
                >
                  {code}
                </button>
              ))}
          ...
          {last ? (
            <>
              ...
              <p className="flex items-baseline gap-1.5">
                <span className="whitespace-nowrap font-mono text-[28px] font-medium leading-none tabular-nums">
                  {lastPicked}
                </span>
              ...
              <div className="h-[5px] overflow-hidden rounded-full border bg-background">
                <div
                  ...
                  style={{ width: `${Math.round((lastPicked / last.requested) * 100)}%` }}
                />
              </div>
      ...
      <div className="flex flex-wrap items-center gap-3 border-t bg-muted/30 px-3.5 py-2.5">
        <span className="whitespace-nowrap font-mono text-[12.5px] font-medium tabular-nums">
        ...
        <span className="whitespace-nowrap font-mono text-[11.5px] text-muted-foreground">{totals.percent}&nbsp;%</span>
