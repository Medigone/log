...
import type { PickLine } from "./usePickScan";

/**
 * Lines to pick.
 * ⚠ Column budget: `30px | minmax(0,1.6fr) | minmax(0,1.1fr) | 152px | 64px`.
 * An earlier version had 7 columns / 482px of fixed tracks; inside the 537px container
 * the two fr tracks collapsed to 18px and 12px and every cell was clipped to one letter.
 * Anything optional (progress, remaining) goes UNDER the item name, not in a new column.
 */
export function PickLinesTable({
  ...
  const visible = lines
    ...
    // complete lines sink to the bottom
    .sort((left, right) => {
  ...
  return (
    <section className="overflow-hidden rounded-xl border bg-card">
      <div className="flex flex-wrap items-center gap-2 border-b px-3 py-2.5">
        ...
        <span className="rounded bg-muted px-1.5 py-0.5 font-mono text-[11px] text-muted-foreground">{lines.length}</span>
        <input
          value={query}
        ...
        return (
          ...
          >
              ...
              <div className="flex min-w-0 items-center gap-1.5">
                <div className="h-1 min-w-6 flex-1 overflow-hidden rounded-full bg-muted">
                  <div
                    ...
                    style={{ width: `${Math.round((value / line.requested) * 100)}%` }}
                  />
                </div>
                <span
                  className={cn(
                    "shrink-0 whitespace-nowrap font-mono text-[10.5px]",
            ...
            <div className="flex min-w-0 flex-col gap-0.5 py-2 pr-2">
              ...
              <span className="truncate font-mono text-[11px] text-muted-foreground/70">{line.salesOrder}</span>
            </div>
            ...
            <div className="flex items-center justify-center gap-1.5 py-2">
              ...
              <span
                className={cn(
                  "whitespace-nowrap font-mono text-[13.5px] font-medium tabular-nums",
